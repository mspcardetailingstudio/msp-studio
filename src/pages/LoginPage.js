...
import { ADMIN_PASSWORD, EMPLOYEE_PASSWORD } from "../config";
...
export default function LoginPage({ onLogin }) {
  ...
  const handleLogin = () => {
    if (role === "admin"    && pass === ADMIN_PASSWORD)    { onLogin("admin");    return; }
    if (role === "employee" && pass === EMPLOYEE_PASSWORD) { onLogin("employee"); return; }
    if (role === "customer") { onLogin("customer"); return; }
    ...
    <div style={{ minHeight:"100vh", background:"#0b1120", display:"flex", alignItems:"center", justifyContent:"center", fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      <div style={{ width:"100%", maxWidth:420, padding:24 }}>
        ...
        <div style={{ background:"#0d1b2e", border:"1px solid #1e3a5f", borderRadius:16, padding:28 }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
              {[
                ...
                <button key={r.id} onClick={() => { setRole(r.id); setPass(""); setErr(""); }}
                  style={{ padding:"10px 8px", borderRadius:8, border:`1px solid ${role===r.id?"#3b82f6":"#1e2d4a"}`, background:role===r.id?"#172554":"#0b1120", color:role===r.id?"#93c5fd":"#475569", fontWeight:600, fontSize:13, cursor:"pointer", transition:"all 0.15s" }}>
          ...
          {/* Password field — only for admin/employee */}
          {role !== "customer" && (
            <div style={{ marginBottom:20 }}>
          ...
          {role === "customer" && (
            <div style={{ marginBottom:20, background:"#0a1628", border:"1px solid #1e3a5f", borderRadius:8, padding:"12px 14px", fontSize:13, color:"#64748b", lineHeight:1.6 }}>
              📋 You'll be taken to the <strong style={{ color:"#93c5fd" }}>Booking Form</strong> to schedule your car detailing appointment. No password needed!
            </div>
          )}
          ...
          <button onClick={handleLogin}
            ...
            {role === "customer" ? "Book an Appointment →" : "Sign In →"}
          </button>
        </div>
      </div>
    </div>
